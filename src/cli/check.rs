...
pub fn run_check(args: CheckArgs) -> i32 {
    ...
    let all_calls: Vec<_> = source_calls.iter().flat_map(|(_, calls)| calls.iter().cloned()).collect();
    ...
    for cat_path in &catalog_paths {
        ...
        let filtered = diagnostics::apply_diag_filter(diags, &diag_cfg);
        for d in filtered {
    ...
    for (uri, pdiags) in diagnostics::check_project(&index, &all_calls) {
        ...
        let filtered = diagnostics::apply_diag_filter(pdiags, &diag_cfg);
        for d in filtered {
            let Some(code) = code_of(&d.code) else { continue };
            findings.push(Finding {
                path: path.clone(),
    ...
    // Filter to given paths when explicit paths were supplied.
    ...
    if !is_machine {
        if findings.is_empty() {
            ...
            println!("{}", color.severity(DiagnosticSeverity::WARNING, &format!("Found {} {}.", findings.len(), if findings.len() == 1 { "error" } else { "errors" })));
        }
    }
