...
fn neovim_config_includes_python_filetype() {
    assert!(
        NEOVIM_LUA.contains("'python'"),
        "filetypes must include python"
    );
}
...
fn neovim_config_includes_jinja_filetype() {
    assert!(
        NEOVIM_LUA.contains("'jinja'"),
        "filetypes must include jinja"
    );
}
...
fn neovim_config_includes_html_filetypes() {
    assert!(
        NEOVIM_LUA.contains("'htmldjango'") && NEOVIM_LUA.contains("'html'"),
...
fn neovim_config_includes_po_filetype() {
    assert!(
        NEOVIM_LUA.contains("'po'"),
        "filetypes must include po (catalog files)"
    );
}
...
fn neovim_config_uses_correct_root_markers() {
    assert!(
        NEOVIM_LUA.contains("'pyproject.toml'"),
        "root_markers must include pyproject.toml"
    );
    assert!(
        NEOVIM_LUA.contains("'.git'"),
...
fn neovim_config_uses_stdio_transport() {
    assert!(
        NEOVIM_LUA.contains("'babel-lsp'") && NEOVIM_LUA.contains("'--stdio'"),
...
fn helix_config_attaches_python() {
    ...
    assert!(
        servers.iter().any(|s| s == "babel-lsp"),
        "python must list babel-lsp"
    );
}
...
fn helix_config_attaches_po() {
    ...
    assert!(
        servers.iter().any(|s| s == "babel-lsp"),
        "po must list babel-lsp (catalog files)"
    );
}
